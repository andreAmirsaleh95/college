...
// YOUR CODE HERE
	struct dnode *newFront = dnode_create();
	dnode_setdata(newFront, ptr);

	if (l->counter == 0) {
		l->back = newFront;
	}else {
		struct dnode *oldNode = l->front;
		oldNode->prev = newFront;
		newFront->next = oldNode;
	}
...
struct dnode *newBack = dnode_create();
	dnode_setdata(newBack, ptr);

	if (l->counter == 0) {
		l->front = newBack;
	}else {
		struct dnode * oldNode = l->back;
		oldNode->next = newBack;
		newBack->prev = oldNode;
	}
...
// YOUR CODE HERE
  struct dnode *currFront = l->front;
  void *ptr = currFront->data;
