 ...
 */
void dlist_add_front(struct dlist *l, size_t s, int location);
 ...
 * @param ptr pointer to generic data to store in new list node
 */
void dlist_add_back(struct dlist *l, size_t s, int location);
 ...
 *
 * @param l pointer to the list (non-NULL)
 * @param s data to find in node
 * @return pointer to data stored in the node that was found node
 */
void *dlist_find_remove(struct dlist *l, int location);
