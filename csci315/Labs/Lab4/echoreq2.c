...
main(int argc, char* argv[]) {
	...
	char *host[BUFFER_SIZE];                // pointer to host name
	...
	Getnameinfo(&sad, sizeof(sad), host, BUFFER_SIZE, serv, BUFFER_SIZE, 0);
	...
	// memcpy(&sad.sin_addr, serv, BUFFER_SIZE);
	...
	char *out_msg = argv[2];
	ssize_t s = Write(sd, out_msg, sizeof(char) * (strlen(out_msg) + 1));
