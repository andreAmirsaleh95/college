...
int main(int argc, char *argv[]) {
	...
	if (fstatvfs(fd, &fs_info) == 0) {
			...
			printf(" file system block size: %d\n", (int) fs_info.f_bsize); // TO-DO (DONE)
			printf(" max. file name length: %d\n", (int) fs_info.f_namemax); // TO-DO (DONE)
	} else {
	...
	if (fstat(fd, &file_info) == 0) {
		...
		// andre's call to getpwuid:
		struct passwd p1;
		struct passwd *p2;
		struct group g1;
		struct group *g2;
		char buf[256];
		getpwuid_r(file_info.st_uid, &p1, buf, sizeof(buf), &p2);
		getpwuid_r(file_info.st_gid, &g1, buf, sizeof(buf), &g2);

		printf(" owner user name = %s\n", p2->pw_name); // TO-DO: man getpwuid (DONE)
		printf(" owner group name = %s\n", g2->gr_name); // TO-DO: man getgrgid (DONE)
		...
		if (S_ISREG(file_info.st_mode)) {
			mode_to_print = "regular file";
		} else if (S_ISDIR(file_info.st_mode)) {
			mode_to_print = "directory";
		} else if (S_ISCHR(file_info.st_mode)) {
			mode_to_print = "character device";
		} else if (S_ISBLK(file_info.st_mode)) {
			mode_to_print = "block device";
		} else if (S_ISLNK(file_info.st_mode)) {
			mode_to_print = "symbolic link";
		} else if (S_ISSOCK(file_info.st_mode)) {
			mode_to_print = "socket";
		} else if (S_ISFIFO(file_info.st_mode)) {
			mode_to_print = "fifo";
		} else {
			mode_to_print = "unknown";
		}
		printf(" mode = %s\n", mode_to_print);
